...
if ($selected_room_id > 0 && isset($rooms[$selected_room_id])) {
  ...
  <main id="main" class="main">
    ...
    <section class="section">
        ...
        <div class="col-lg-12">
          ...
          <div class="card mt-4">
            <div class="card-body">
              ...
              <ul class="nav nav-tabs mb-3" id="tariffViewTabs" role="tablist">
              ...
              <div class="tab-content" id="tariffViewTabsContent">
                <!-- List View Tab -->
                ...
                <div class="calendar-container">
                  ...
                  <div class="mt-3">
                    ...
                    <div class="d-flex flex-wrap gap-3">
                      <div><span class="badge bg-primary">Weekday</span></div>
                      <div><span class="badge bg-warning">Weekend</span></div>
                      <div><span class="badge bg-danger">Holiday</span></div>
                      <div><span class="badge bg-success">All Days</span></div>
                    </div>
                  </div>
                </div>
              <?php endif; ?>
      ...
      <?php endif; ?> <!-- Close the if ($selected_room) conditional -->
    </section>
  </main><!-- End #main -->
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      if (document.getElementById('tariffCalendar')) {
        // Initialize calendar variables
        let currentDate = new Date();
        let currentMonth = currentDate.getMonth();
        let currentYear = currentDate.getFullYear();
        
        // Get tariff data from PHP to JavaScript
        const tariffData = <?php 
          if (!empty($room_tariffs)) {
            $tariffJson = [];
            foreach ($room_tariffs as $tariff) {
              $tariffJson[] = [
                'id' => $tariff['id'],
                ...
                'price' => $tariff['price'],
                'discount_price' => $tariff['discount_price'],
              ];
            }
            echo json_encode($tariffJson);
          } else {
            echo '[]';
          }
        ?>;
        ...
        function generateCalendar(month, year) {
          ...
          const calendarBody = document.querySelector('#tariffCalendar tbody');
          ...
          for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
              ...
              if (i === 0 && j < firstDay) {
                ...
                dateDiv.classList.add('d-flex', 'justify-content-between', 'mb-1');
                ...
                if (date === todayDate.getDate() && month === todayDate.getMonth() && year === todayDate.getFullYear()) {
                  dateSpan.classList.add('text-primary', 'fw-bold');
                }
                
                dateDiv.appendChild(dateSpan);
                cell.appendChild(dateDiv);
                
                // Check for tariffs on this date
                const currentDateObj = new Date(year, month, date);
                const currentDateStr = currentDateObj.toISOString().split('T')[0];
                
                // Find applicable tariffs for this date
                const applicableTariffs = tariffData.filter(tariff => {
                  const startDate = new Date(tariff.start_date);
                  const endDate = new Date(tariff.end_date);
                  return currentDateObj >= startDate && currentDateObj <= endDate;
                });
                
                // Sort by day_type priority (all < weekday/weekend < holiday)
                applicableTariffs.sort((a, b) => {
                  const priorityMap = { 'all': 1, 'weekday': 2, 'weekend': 2, 'holiday': 3 };
                  return priorityMap[b.day_type] - priorityMap[a.day_type];
                });
                
                // Get the highest priority tariff for this date
                const tariff = applicableTariffs.length > 0 ? applicableTariffs[0] : null;
                ...
                if (tariff) {
                  ...
                  dayTypeBadge.classList.add('badge', badgeClass, 'd-block', 'mb-1');
                  dayTypeBadge.textContent = tariff.day_type.charAt(0).toUpperCase() + tariff.day_type.slice(1);
                  priceDiv.appendChild(dayTypeBadge);
                  
                  // Add price
                  const priceSpan = document.createElement('div');
                  priceSpan.classList.add('fw-bold');
                  priceSpan.textContent = '₹' + parseFloat(tariff.price).toLocaleString('en-IN');
                  priceDiv.appendChild(priceSpan);
                  
                  // Add discount price if available
                  if (tariff.discount_price) {
                    const discountSpan = document.createElement('div');
                    discountSpan.classList.add('text-success', 'small');
                    discountSpan.textContent = 'Discount: ₹' + parseFloat(tariff.discount_price).toLocaleString('en-IN');
                  ...
                  editBtn.classList.add('btn', 'btn-sm', 'btn-outline-primary', 'mt-1');
                  editBtn.innerHTML = '<i class="bi bi-pencil"></i>';
                  editBtn.setAttribute('data-bs-toggle', 'modal');
                  editBtn.setAttribute('data-bs-target', '#editTariffModal');
                  editBtn.setAttribute('data-id', tariff.id);
                  
                  // Add click event to populate edit modal
                  editBtn.addEventListener('click', function() {
                    // Find the edit button in the list view with the same tariff ID
                    const listEditBtn = document.querySelector(`.edit-tariff-btn[data-id="${tariff.id}"]`);
                    if (listEditBtn) {
                      // Trigger click on the list view edit button to populate the modal
                      listEditBtn.click();
                    }
                  });
            ...
            // Stop creating rows if we've gone through all days of the month
            if (date > daysInMonth) {
              break;
            }
          }
        ...
        // Add event listener for tab change to refresh calendar
        ...
        if (calendarTab) {
          ...
          // Also listen for the Bootstrap tab event
          calendarTab.addEventListener('shown.bs.tab', function() {
            generateCalendar(currentMonth, currentYear);
          });
        }
        
        // Force calendar generation on page load if calendar tab is active
